...
export default function CalendarioPage() {
  ...
  useEffect(() => {
    calendarService.list().then(setEvents).catch(() => setEvents([]));
  }, []);
    ...
    <LayoutDesktop>
      ...
      {modal && <div role="dialog" aria-modal="true" aria-labelledby="calendar-modal-title" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
          <div className="mb-6 flex items-start justify-between gap-4"><div><h2 id="calendar-modal-title" className="text-xl font-bold text-gray-800">{modal.type === "create" ? "Agendar manutenção" : "Detalhes do evento"}</h2></div><button type="button" aria-label="Fechar modal" onClick={closeModal} className="cursor-pointer rounded-md p-1 text-gray-500 hover:bg-gray-100"><X className="h-5 w-5" /></button></div>
