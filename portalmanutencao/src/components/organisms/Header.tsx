...
export default function Header() {
    ...
    const [notifications, setNotifications] = useState([
        { id: 1, title: "Ocorrência Aprovada", about: "A ocorrência do Professor Manutenção foi validada.", isUnread: true },
        { id: 2, title: "Sistema Atualizado", about: "O portal recebeu uma nova versão hoje.", isUnread: false },
        { id: 3, title: "Manutenção Concluída", about: "Máquina Torno CNC liberada para uso.", isUnread: true },
    ]);
            ...
            <nav className="max-full mx-auto flex items-center justify-between">
                ...
                <div>
                    <ul className="flex items-center gap-4">
                        <li>
                            <button className="transition-colors hover:bg-white/10 h-12 w-12 flex justify-center items-center rounded-lg cursor-pointer" aria-label="Ajuda">
                        ...
                        <li ref={dropdownRef} className="relative">
                            <button
                                ...
                                className={`relative transition-colors hover:bg-white/10 h-12 w-12 flex justify-center items-center rounded-lg cursor-pointer ${isNotificationOpen ? 'bg-white/20' : 'hover:bg-white/10'}`}
                            ...
                            {isNotificationOpen && (
                                <div className="absolute right-0 top-full mt-2 w-80 bg-[#FAFAFA] text-weg-blue rounded-lg shadow-2xl border border-weg-blue z-50 flex flex-col">
                                    ...
                                    <div className="max-h-75 overflow-y-auto">
                                        {notifications.map((notif) => (
                                            <NotificationItem 
                                                ...
                                                onClick={() => console.log(`Clicou na notificação ${notif.id}`)}
                                            />
                                        ))}
                                    </div>
                                    ...
                                    <div className="group px-4 py-2.5 hover:bg-weg-blue/85 hover:text-[#FAFAFA] rounded-b-md cursor-pointer flex items-center justify-between transition-colors">
                                        ...
                                        <span className="text-xs text-gray-500 group-hover:text-white/80">Ctrl+N</span>
                                    </div>
                                </div>
                            )}
                        </li>
                        ...
                        <li>
                            <button className="transition-colors hover:bg-white/10 h-12 w-12 flex justify-center items-center rounded-lg cursor-pointer" aria-label="Configurações">
