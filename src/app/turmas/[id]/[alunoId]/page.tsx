...
export default function StudentPage({ params }: Props) {
  ...
  useEffect(() => {
    async function loadStudent() {
      try {
        ...
        const loadedClassGroups = await Promise.all(loadedStudent.classGroupIds.map((classGroupId) => classGroupBrowserService.getById(classGroupId)));
  ...
  const currentClassGroup = classGroups.find((classGroup) => classGroup.id === id);

  return (
    <LayoutDesktop breadcrumbLabels={{
      1: currentClassGroup ? `Turma ${currentClassGroup.acronym}` : "Turma",
      2: student.name,
    }}>
