...
export default function EditClassPage({ params }: PageProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    try {
      await classGroupBrowserService.updateAcronym(id, result.output.acronym);
      toast.success("Turma atualizada com sucesso.");
      router.push(`/turmas/${id}`);
      router.refresh();
    } catch (error) {
      toast.error(getServiceErrorMessage(error, "Não foi possível salvar as alterações da turma."));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <LayoutDesktop>
      <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-8">
        <div className="flex items-center gap-3"><Link href={`/turmas/${id}`} aria-label="Voltar para a turma"><Button type="button" variant="secondary" icon={ArrowLeft}>Voltar</Button></Link><div><h1 className="text-2xl font-bold text-gray-900 md:text-3xl">Editar turma</h1><p className="mt-1 text-sm text-gray-500">Atualize a sigla da turma.</p></div></div>
        {loading ? <div className="rounded-xl border border-gray-200 bg-white p-8 text-center text-gray-500 shadow-sm">Carregando dados da turma...</div> : (
          <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex justify-end gap-3 border-t border-gray-100 pt-4"><Link href={`/turmas/${id}`}><Button type="button" variant="secondary">Cancelar</Button></Link><Button type="submit" icon={Save} disabled={saving}>{saving ? "Salvando..." : "Salvar alterações"}</Button></div>
          </form>
        )}
      </div>
    </LayoutDesktop>
